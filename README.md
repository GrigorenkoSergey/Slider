...
| Опция | Описание |
|-------|----------|
| `min: number` | Минимальное значение шкалы диапазона |
| `max: number` | Максимальное значение шкалы диапазона |
| `selector: string` | CSS-селектор выбора блока, содержащего наш слайдер |

## Дополнительные опции
| Опция | Описание |
|-------|----------|
| `step: number` | По умолчанию 1/100. Значения бегунка всегда кратны шагу. |
| `partsNum: number` | По умолчанию 2. На сколько частей делим интерактивную шкалу |
| `thumbLeftPos: number` | По умолчанию 0. Можем установить положение левого бегунка |
...
| `hintAlwaysShow: boolean ` | По умолчанию `false`. Если установлено в `true`, подсказка показывается всегда. При этом значение `hintAlwaysShow` игнорируется. |
| `angle: number` | По умолчанию 0. Поворот слайдера. Принимает значения от 0 до 90. |
| `showScale: boolean` | По умолчанию `true`. Под слайдером отображаются значения шкалы, кратные шагу. Их колличество зависит от свойства `partsNum`. При щелчке по ним ближайший кругляш бежит к этому значению. При желании можно поменять отображение значений с помощью свойства `alternativeRange`. |
