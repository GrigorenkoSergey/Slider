...
# Подключение 
1. Через **webpack**. Либо скачай полностью проект, если нужно добавить функционал и подправить стили, либо только файлы **slider.js** и **sligit der.css**.
...
Сам по себе бегунок бесполезен. Он должен быть связан с какими-либо данными и его перемещение должно вызывать какие-то дополнительные  полезные действия. К примеру, пользователь, двигая круглящ, меняет возможные значения срока ипотеки. Какая-то сторонняя функция высчитывает итоговую сумму, которую он заплатит и отображает в каком-то поле *elemDom*. Таких элементов может быть сколько угодно.
