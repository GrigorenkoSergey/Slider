...
| `partsAmount: number` | По умолчанию 2. На сколько частей (не всегда одинаковых) делим интерактивную шкалу. Максимальное значение 100.|
...
## API
### slider(method: 'getOptions' | 'setOptions' | 'getOffsets' | 'onChange', options)
Альтернативный вызов методов API. Если передан несуществующий метод, будет выброшена ошибка.
```ts
const instance = new Slider(opts);

// вызовы эквивалентны
instance.slider('setOptions', options);
