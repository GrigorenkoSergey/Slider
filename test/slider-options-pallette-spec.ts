...
describe('Реагирует на ручное изменение положения бегунков\n', () => {
  ...
  it('При перетаскивании бегунков мышкой, значение соответствующего поля меняется', () => {
    ...
    slider.setOptions({ thumbLeftValue: 0 });
...
describe('Данные баги более не возникают\n', () => {
  ...
  it('При смене значения "min" c "0.5" на "0" и обратно неправильно высчитываются значения якорей', () => {
  ...
  it('При смене точности до 3 знаков, значения якорей шкалы не совпадают со значениями модели', () => {
  ...
  it(`При установке опции "range=false" значение поля "thumbRightValue" доступно
    для редактирования`, () => {
    slider.setOptions({ range: false });
    if (inputs.thumbRightValue === null) throw new Error();
  ...
  it(`При выборе опции "hintAlwaysShow" опция "hintAboveThumb" доступна для
    редактирования`, () => {
    if (inputs.hintAboveThumb === null) throw new Error();
  ...
  it('При наложении бегунков подсказоки не объединяются', () => {
